...
const joinSettings = fs.readFileSync(path.join(root, 'app', 'components', 'projects', 'ProjectJoinSettings.js'), 'utf8');
...
assert.match(workspace, /detailTriggerRef\.current\?\.isConnected/);
assert.match(workspace, /<ProjectTopicFilters/);
assert.match(workspace, /deriveReportableTopics/);
assert.doesNotMatch(workspace, /reportableMine/);
assert.match(workspace, /visibleTopics=\{topicFilterResult\.visibleTopics\}/);
assert.match(workspace, /ScheduleStructure project=\{project\} topics=\{topics\}/, 'Timeline scheduling context must retain all topics');
...
assert.doesNotMatch(manage, /InviteForm|projects\/invitations/, 'People settings should use only the global join-link flow');
assert.match(joinSettings, /joinHint/);
assert.match(joinSettings, /action:'approve',role:/);
...
assert.match(projectsCore, /project\.role === 'viewer'\) throw codedError\('project_forbidden'\)/);
