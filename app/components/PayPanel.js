 ...
 */
export default function PayPanel({ code, payTo, amountSatang, periodId, pending, busy, call, onError }) {
	const { t, fmt } = useCopy();
	const [flash, setFlash] = useState('');
	const [saving, setSaving] = useState(false);
	const [attaching, setAttaching] = useState(false);

	const qrUrl = `/api/megu/a/${code}/qr?amount=${amountSatang}${periodId ? `&period=${periodId}` : ''}`;
	...
	async function saveQr() {
		...
		try {
			...
			const filename = `megu-${code}-${(amountSatang / 100).toFixed(2)}.png`;
	...
	async function attachSlip(slip) {
		...
		setAttaching(true);
		try {
			await call('POST', `/payments/${pending.id}/slip`, { dataUrl: slip.dataUrl, ref: slip.ref });
		}
		finally {
			setAttaching(false);
		}
			...
			<section className="panel pay-panel">
				...
				<div className="pay-body">
					<div className="pay-figure">
						<span className="pay-amount">{fmt.money(amountSatang)}</span>
					...
					{!pending && (
						<button type="button" className="btn btn-pay btn-block" disabled={busy} onClick={() => call('POST', '/pay', periodId ? { periodId } : undefined)}>
							{t.pay.iHavePaid}
						</button>
					)}
			...
			<div className="pay-body">
				...
				<figure className="pay-qr">
					{/* A plain <img> and not a background image, so that a long
					    ...
					    being involved. */}
					<img src={qrUrl} alt={`${t.pay.scanHint} — ${fmt.money(amountSatang)}`} width={640} height={640} />
				</figure>
				...
				<div className="pay-fallback">
					...
					<div className="pay-copy-row">
						<span className="pay-number">{fmt.money(amountSatang)}</span>
						<button type="button" className="btn btn-sm btn-secondary" onClick={() => copy((amountSatang / 100).toFixed(2), t.pay.amountCopied)}>
							{t.pay.copyAmount}
						</button>
					</div>
				</div>
				...
				{!pending ? (
					<button
						...
						disabled={busy}
						onClick={() => call('POST', '/pay', periodId ? { periodId } : undefined)}
					>
						{t.pay.iHavePaid}
					</button>
				) : (
								...
								<div className="slip-state">
									<span className={`chip ${pending.slipVerdict === 'duplicate' ? 'chip-due' : 'chip-clear'}`}>
										{pending.slipVerdict === 'duplicate'
											...
											: pending.slipVerdict === 'matched' ? t.slip.verdictMatched : t.slip.verdictUnread}
									</span>
									<SlipPicker
