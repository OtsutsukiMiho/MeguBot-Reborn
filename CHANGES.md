...
| **Two processes built the schema at once** | `index.js` forks the bot and the web process and both call `initCoreSchema()`, with every statement committing on its own. Unguarded that races — and once a rename is in the mix it stops being harmless, because both processes see the old name present, both issue the rename, and the second finds the table it named already gone. The loser logs "schema init failed", drops activity features, and leaves a part-migrated database behind. Now one transaction behind a Postgres advisory lock. | `core/schema.js` |
